...
files_in = [os.path.join(
    dir_source,
    '0d25_daily/Globsnow_SWE/v3/Data/SWE.1440.720.{:4d}.nc'.format(y))
    for y in years_targets
]
file_out = os.path.join(
    dir_target, 'processed/1d/daily/swe.nc'
)
...
for p in files_in:
    ...
    d = xr.open_dataset(p).rename({'SWE': 'val'})
    #d = d.where(d.val != -2, np.nan)
    #d = d.where(d.val != -1, np.nan)
    d = d.where(d.val >= 0, np.nan)
    swe_stack.append(d)
...
swe_num_missing = swe_missing.rolling(time=24, center=True).sum().sel(
