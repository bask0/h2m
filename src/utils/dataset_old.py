...
class SpatialData(object):
    ...
    def __init__(
        ...
        with xr.open_dataset(self._path) as d:
            ...
            if self._hastime:
                ...
                self._time_d = (pd.DatetimeIndex(self._time) - pd.datetime(1970, 1, 1)).days.values
    ...
    def get_time_coverage(
        ...
        and upper bound per time step that covers the 'time_range'. This bounds indicate the indices
        ...
        if warmup_freq == 'D':
            ...
            warmup_steps = len(pd.date_range(date_start, warmup_start_date)) - 1
        else:
...
class Bucket(Dataset):
    ...
    def set_space_ids(
            self,
            cv_mask_indices: Dict[str, Iterable[int]]) -> None:
    ...
    def get_sampler(self, sample_set: str) -> 'Bucket':
        ...
        if sample_set not in ['train', 'valid', 'test']:
            raise ValueError('Arg ``sample_set`` must be on of {``train``, ``valid``, ``test``} but '
                             f'is ``{sample_set}``.')
        ...
        sampler._feature_time = sampler.getvartime(ds)[time_slice[0]:time_slice[1]]
    ...
    def read_data(self, lat: int, lon: int, standardize: str = 'features') -> Dict[str, Any]:
        ...
        for ds_key, ds_value in self._datasets.items():
            ...
            if standardize == 'all':
                ...
                raise ValueError(
                    'Argument ``standardize`` must be one of {"all", "targets", "none"}.')
            ...
            try:
                if hastime:
                    ...
                    if standardize_this:
                        data = (ds['data'][t0:t1, ..., lat, lon][..., np.newaxis] - m) / s
                    else:
                        data = ds['data'][t0:t1, ..., lat, lon][..., np.newaxis]
    ...
    def _get_random_year_ind(
            self,
            data_time: Dict[str, Iterable],
            slices: Dict[str, Iterable]) -> np.array:
        """Get indices for a random years for train, validation or test set.
        ...
        ----------
        data_time
            Dataset time dimensions, must have key set in self.rep_years_ref_var.
        slices
            Dataset slices, must have key set in self.rep_years_ref_var.

        Returns
        ----------
        A tuple
            - Indices (np.array) corresponding time dimension indices.
            - n_cutoff: The length of the indices array. This is used to cut the spin-up period from predictions.

        """

        if self._sample_set is None:
            raise ValueError(
                'You cannot get the bins for this bucket, use my_sampler = my_bucket.get_sampler(sample_set).')

        t = data_time[slices[0]:slices[1]]

        if (self.rep_years_n is None) or (self.rep_years_n == 0):
            indices = np.arange(len(t))
            n_cutoff = 0
        else:
            years = np.unique(t.astype('datetime64[Y]')).astype(str)
            # Remove leap years bc this leads to different length batches.
            years = np.setdiff1d(years, np.arange(1972, 2050, 4).astype(str))

            indices = np.empty(0, dtype=int)
            for i in range(self.rep_years_n):
                ...
                start_idx = np.argwhere(t == start_date)[0][0]
                end_idx = np.argwhere(t == end_date)[0][0] + 1

                indices = np.concatenate(
                    (indices, np.arange(start_idx, end_idx)))

            n_cutoff = len(indices)
            indices = np.concatenate((indices, np.arange(len(t))))

        return indices, n_cutoff
    ...
    def __getitem__(self, idx: int) -> Tuple[Dict, Tuple[int, int]]:
        ...
        data = self.read_data(self.lat, self.lon)
    ...
    def __repr__(self) -> str:
        summary = ['Bucket @ {}'.format(self._path), '-' * (len(self._path) + 12)]
...
class BucketCollator(object):
    ...
    def __init__(self, sampler: Bucket, n_rep_years: int):
        ...
        years = np.unique(self.time.astype(
    ...
    def __call__(self, batch):
        ...
        indices, n_cutoff = self.get_random_year_ind()

        batch, locations = self.collate(batch)

        for k, v in batch.items():
            if k in self.dynamic_features:
                batch[k] = v[:, indices, ...]

        return batch, locations, n_cutoff
    ...
    def get_random_year_ind(self) -> np.array:
        ...
        if (self.n_rep_years is None) or (self.n_rep_years == 0):
            ...
            n_cutoff = len(indices)
            indices = np.concatenate((indices, np.arange(len(self.time))))
